/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package Clases.EvaluacionSupervisores;

import Clases.EvaluacionOperativo.*;
import java.util.Date;

/**
 *
 * @author Jorge Luis
 */
public class ClassEvaluacionSupervisores {
    
    private int id_listaempleados;
    private Date fecha;
    private int face;
    private int noEvaluacion;
    private int estado_evaluacion;
    private String nombres;
    private String apellidos;
    private String depto;
    private String Puesto;
    private String fechaS;
    private int codigo;
    private int id_evaluacion;
    private String faceS;
    private int UNO;
    private int DOS;
    private int TRES;
    private int CUATRO;
    private int UNO1;
    private int DOS1;
    private int TRES1;
    private int CUATRO1;
    private int UNO2;
    private int DOS2;
    private int TRES2;
    private int CUATRO2;
    private int UNO3;
    private int DOS3;
    private int TRES3;
    private int CUATRO3;
    private int UNO4;
    private int DOS4;
    private int TRES4;
    private int CUATRO4;
    private String NOTA;

    public String getNOTA() {
        return NOTA;
    }

    public void setNOTA(String NOTA) {
        this.NOTA = NOTA;
    }
    public int getNoEvaluacion() {
        return noEvaluacion;
    }

    public void setNoEvaluacion(int noEvaluacion) {
        this.noEvaluacion = noEvaluacion;
    }
    public int getUNO() {
        return UNO;
    }

    public void setUNO(int UNO) {
        this.UNO = UNO;
    }

    public int getDOS() {
        return DOS;
    }

    public void setDOS(int DOS) {
        this.DOS = DOS;
    }

    public int getTRES() {
        return TRES;
    }

    public void setTRES(int TRES) {
        this.TRES = TRES;
    }

    public int getCUATRO() {
        return CUATRO;
    }

    public void setCUATRO(int CUATRO) {
        this.CUATRO = CUATRO;
    }

    public int getUNO1() {
        return UNO1;
    }

    public void setUNO1(int UNO1) {
        this.UNO1 = UNO1;
    }

    public int getDOS1() {
        return DOS1;
    }

    public void setDOS1(int DOS1) {
        this.DOS1 = DOS1;
    }

    public int getTRES1() {
        return TRES1;
    }

    public void setTRES1(int TRES1) {
        this.TRES1 = TRES1;
    }

    public int getCUATRO1() {
        return CUATRO1;
    }

    public void setCUATRO1(int CUATRO1) {
        this.CUATRO1 = CUATRO1;
    }

    public int getUNO2() {
        return UNO2;
    }

    public void setUNO2(int UNO2) {
        this.UNO2 = UNO2;
    }

    public int getDOS2() {
        return DOS2;
    }

    public void setDOS2(int DOS2) {
        this.DOS2 = DOS2;
    }

    public int getTRES2() {
        return TRES2;
    }

    public void setTRES2(int TRES2) {
        this.TRES2 = TRES2;
    }

    public int getCUATRO2() {
        return CUATRO2;
    }

    public void setCUATRO2(int CUATRO2) {
        this.CUATRO2 = CUATRO2;
    }

    public int getUNO3() {
        return UNO3;
    }

    public void setUNO3(int UNO3) {
        this.UNO3 = UNO3;
    }

    public int getDOS3() {
        return DOS3;
    }

    public void setDOS3(int DOS3) {
        this.DOS3 = DOS3;
    }

    public int getTRES3() {
        return TRES3;
    }

    public void setTRES3(int TRES3) {
        this.TRES3 = TRES3;
    }

    public int getCUATRO3() {
        return CUATRO3;
    }

    public void setCUATRO3(int CUATRO3) {
        this.CUATRO3 = CUATRO3;
    }

    public int getUNO4() {
        return UNO4;
    }

    public void setUNO4(int UNO4) {
        this.UNO4 = UNO4;
    }

    public int getDOS4() {
        return DOS4;
    }

    public void setDOS4(int DOS4) {
        this.DOS4 = DOS4;
    }

    public int getTRES4() {
        return TRES4;
    }

    public void setTRES4(int TRES4) {
        this.TRES4 = TRES4;
    }

    public int getCUATRO4() {
        return CUATRO4;
    }

    public void setCUATRO4(int CUATRO4) {
        this.CUATRO4 = CUATRO4;
    }
    
    
    
    
    
    
    
    
    
    
    
    
    
    public String getFaceS() {
        return faceS;
    }

    public void setFaceS(String faceS) {
        this.faceS = faceS;
    }
    
    public String getFechaS() {
        return fechaS;
    }

    public void setFechaS(String fechaS) {
        this.fechaS = fechaS;
    }

    
    
    public int getId_evaluacion() {
        return id_evaluacion;
    }

    public void setId_evaluacion(int id_evaluacion) {
        this.id_evaluacion = id_evaluacion;
    }
    
    public int getCodigo() {
        return codigo;
    }

    public void setCodigo(int codigo) {
        this.codigo = codigo;
    }
    

    public String getNombres() {
        return nombres;
    }

    public void setNombres(String nombres) {
        this.nombres = nombres;
    }

    public String getApellidos() {
        return apellidos;
    }

    public void setApellidos(String apellidos) {
        this.apellidos = apellidos;
    }

    public String getDepto() {
        return depto;
    }

    public void setDepto(String depto) {
        this.depto = depto;
    }

    public String getPuesto() {
        return Puesto;
    }

    public void setPuesto(String Puesto) {
        this.Puesto = Puesto;
    }
    
    
    
    //private int Organizacion;
    //private int Liderasgo;
    //private int Presentacion;
    //private int Responsabilidad;
    //private int Iniciativa;
    //private int estado;
    //private int tipo;

    public int getId_listaempleados() {
        return id_listaempleados;
    }

    public void setId_listaempleados(int id_listaempleados) {
        this.id_listaempleados = id_listaempleados;
    }

    public Date getFecha() {
        return fecha;
    }

    public void setFecha(Date fecha) {
        this.fecha = fecha;
    }

    public int getFace() {
        return face;
    }

    public void setFace(int face) {
        this.face = face;
    }

    public int getEstado_evaluacion() {
        return estado_evaluacion;
    }

    public void setEstado_evaluacion(int estado_evaluacion) {
        this.estado_evaluacion = estado_evaluacion;
    }
   
}
